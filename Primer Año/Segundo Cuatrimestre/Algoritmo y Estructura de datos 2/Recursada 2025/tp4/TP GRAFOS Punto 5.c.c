...
void eliminarVertice(tGrafoNoPonderado*, tVertice);
...
void agregarArco(tGrafoNoPonderado* pGrafo, tArco pArco) {
  if (pGrafo->vertices[pArco.origen] == 1 && pGrafo->vertices[pArco.destino] == 1) {
    ...
    printf("No se pudo agregar el arco [%d, %d]\n", pArco.origen, pArco.destino);
  }
}

void eliminarVertice(tGrafoNoPonderado* pGrafo, tVertice pVertice) {
  if (pGrafo->vertices[pVertice] != 0) {
    pGrafo->vertices[pVertice] = 0;
    printf("Se elimino el vertice %d\n", pVertice);
  } else {
    printf("No se pudo eliminar el vertice %d\n", pVertice);
  }
}
