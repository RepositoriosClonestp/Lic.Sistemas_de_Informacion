...
typedef struct {
	tNodo * principio; /*un puntero al primer nodo de la cola.*/
	tNodo * final; /* un puntero al último nodo de la cola.*/
}tCola;

/* Declaración de prototipos */
void inicializarCola();
bool colaVacia( tCola );
void push( tDatos ); /* insertar un elemento al final de la cola*/
void pop();	/* quita un elemento del frente de la cola*/
void visualizarElementos( tCola );
tNodo * primerElemento( tCola ); /* Retorna un puntero al primer nodo de la cola*/
tDatos primerElemento2( tCola ); /* Retorna los datos del primer nodo*/
...
int main() {
	...
	primero = primerElemento(cola);	/* Obtiene el primer nodo de la cola y, si no está vacío, imprime sus datos.*/
	if(primero != NULL) {
	...
	primeroB = primerElemento2(cola); /*obtener los datos del primer nodo y los imprime.*/
...
void push( tDatos pDatos ) {	
	...
	/* 3. 	Asignar valores a la parte de datos del nodo 
			y al campo siguiente y lo marca como el último */
