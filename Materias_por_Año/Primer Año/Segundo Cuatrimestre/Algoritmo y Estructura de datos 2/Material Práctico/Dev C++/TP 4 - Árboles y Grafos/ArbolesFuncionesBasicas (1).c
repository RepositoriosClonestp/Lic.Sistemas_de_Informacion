...
int main() {
	...
	dato.codigo = 80;
	strcpy(dato.descripcion, "Descripcion 80");	
	insertarElemento(&arbol, dato);
	
	dato.codigo = 78;
	strcpy(dato.descripcion, "Descripcion 78");	
	insertarElemento(&arbol, dato);
		
	dato.codigo = 79;
	strcpy(dato.descripcion, "Descripcion 79");	
	insertarElemento(&arbol, dato);
		
	dato.codigo = 84;
	strcpy(dato.descripcion, "Descripcion 84");	
	insertarElemento(&arbol, dato);
...
void eliminarElementoOtraManera( tArbol ** pArbol, int pCodigo ) {
	tArbol * aux;
	...
	if( arbolVacio(*pArbol)) {
		...
		if( pCodigo < (*pArbol)->datos.codigo ) {
			eliminarElementoOtraManera( &((*pArbol)->hijoIzquierdo) , pCodigo);
		} else {
			if( pCodigo > (*pArbol)->datos.codigo ) {
				eliminarElementoOtraManera( &((*pArbol)->hijoDerecho) , pCodigo);
			} else {
				// Se encontró el nodo que se desea eliminar
				if( (*pArbol)->hijoIzquierdo == NULL) {
					/* Por acá se eliminaría si es una hoja o si tiene un hijo derecho */
					...
					*pArbol = (*pArbol)->hijoDerecho;
					...
					if( (*pArbol)->hijoDerecho == NULL) {
						...
						*pArbol = (*pArbol)->hijoIzquierdo;
						...
						(*pArbol)->datos = aux->datos;
						eliminarElementoOtraManera( &((*pArbol)->hijoIzquierdo) , (*pArbol)->datos.codigo );
					} 										
				}
			}
		}		
